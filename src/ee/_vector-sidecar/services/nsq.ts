...
export class NSQClient {
  ...
  public connect(): Promise<any> {
    this.writer = new Promise((resolve, reject) => {
      ...
      w.connect();
      w.on('ready', () => {
        console.info(`NSQ writer connected to ${this.host}:${this.tcpPort}`);
        resolve(w);
      });
      w.on('closed', () => {
        console.error(`NSQ writer disconnected from ${this.host}:${this.tcpPort}`);
        delete this.writer;
      });
      w.on('error', (err) => {
        console.error(`NSQ writer ${this.host}:${this.tcpPort} : ${err.message}`);
  ...
  public consume(topic: string, channel: string, handle: (msg: Message) => void, opts: ConsumerOptions) {
    ...
    reader.connect();
    reader.on('error', (err) => {
      console.warn(`NSQ consumer ${topic}.${channel}: ${err.message}`);
    });
    reader.on('discard', () => {
      console.warn(`NSQ discarding message on ${topic}.${channel} after ${opts.maxAttempts} attempts`);
    });
    reader.on('nsqd_connected', () => {
      console.info(`NSQ consumer ${topic}:${channel} connected to ${this.host}:${this.tcpPort}`);
    });
    reader.on('nsqd_closed', () => {
      console.warn(`NSQ consumer ${topic}:${channel} disconnected from ${this.host}:${this.tcpPort}`);
