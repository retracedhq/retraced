...
export function startStatusPageReporter(
  ...
  const reporter = new StatusPageReporter(getRegistry(), url, pageId, statusPageToken, metricIds);
  logger.info("created");
...
export function bootstrapFromEnv() {
  ...
  if (!statsdHost) {
    logger.error(
      "neither KUBERNETES_SERVICE_HOST nor STATSD_HOST is set, metrics will not be reported to statsd or sysdig"
    );
  } else {
  ...
  startStatusPageReporter(statusPageUrl, statusPagePageId, statusPageToken, metricIds, intervalMs);
}
...
export function histogram(name: string, help?: string, labels?: string[]) {
  ...
  const summary =
    (Prometheus.register.getSingleMetric("summaries") as Prometheus.Summary<string>) ||
  ...
  return {
    update(value: number, obsLabels?: string[]) {
      ...
      summary.labels(...(obsLabels || name.split("_"))).observe(value);
    },
  };
}
...
export function timer(name: string, help?: string, labels?: string[]) {
  ...
  const throughput =
    (Prometheus.register.getSingleMetric("counters") as Prometheus.Counter<string>) ||
  ...
  return {
    update(nanos: number, obsLabels?: string[]) {
      ...
      wl.inc();
      times.labels(...(obsLabels || name.split("_"))).observe(nanos);
    },
  };
}
...
export function meter(name: string, help?: string, labels?: string[]) {
  ...
  return {
    mark(count?: number, obsLabels?: string[]) {
      ...
      throughput.labels(...(obsLabels || name.split("_"))).inc(count);
    },
  };
}
...
export function gauge(name: string, help?: string, labels?: string[]) {
  ...
  const gaugeObj =
    (Prometheus.register.getSingleMetric("gauges") as Prometheus.Gauge<string>) ||
  ...
  return {
    set(value: number, obsLabels?: string[]) {
      ...
      gaugeObj.labels(...(obsLabels || name.split("_"))).set(value);
    },
  };
}
...
export async function instrument(name: string, delegate: () => any) {
  ...
  try {
    ...
    incrementOtelCounter("method.errors", 1, { "class.method": name });
    errors.mark();
    throw err;
  } finally {
    const elapsed = process.hrtime(start);
    const elapsedNanos = elapsed[0] * 1000000000 + elapsed[1];
    recordOtelHistogram("method.executionTime", elapsedNanos, { "class.method": name });
