...
@suite class SwaggerTest {
    @test public "swagger.removeRoutesNotMatching"() {
        ...
        let res = expect(swagger.paths["/admin/v1/project/{projectId}/apitoken"]).not.to.exist;
        res = expect(swagger.paths["/publisher/v1/project/{projectId}/event"]).to.exist;
        return res;
    }
    ...
    @test public "swagger.filterAndAssign"() {
        ...
        let res = expect(specs[0].swagger.paths["/project/{projectId}/event"]).to.exist;
        res = expect(specs[0].swagger.paths["/project/{projectId}/apitoken"]).not.to.exist;
        res = expect(specs[0].swagger.basePath).to.equal("/publisher/v1");
        res = expect(specs[0].swagger.info.description).to.equal(specs[0].description);
        res = expect(specs[0].swagger.info.title).to.equal(specs[0].title);

        res = expect(specs[1].swagger.paths["/project/{projectId}/event"]).not.to.exist;
        res = expect(specs[1].swagger.paths["/project/{projectId}/apitoken"]).to.exist;
        res = expect(specs[1].swagger.basePath).to.equal("/admin/v1");
        res = expect(specs[1].swagger.info.description).to.equal(specs[1].description);
