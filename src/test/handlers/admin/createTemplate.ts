...
import { deprecated } from "../../../handlers/admin/createTemplate";

@suite class CreateTemplate {
    @test public async "CreateTemplate#createTemplate()"() {
        let pool = getPgPool();
        try {
            await cleanup(pool);
            let res = await setup(pool);
            let result = await createTemplate(`id=${res.id} token=${res.token}`, "test", "test", {
                id: "test",
                name: "test",
                rule: "test",
                template: "test",
    ...
    @test public async "CreateTemplate#deprecated()"() {
        let pool = getPgPool();
        try {
            await cleanup(pool);
            let res = await setup(pool);
