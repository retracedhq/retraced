...
import { deprecated } from "../../../handlers/admin/createEnvironment";

@suite class CreateEnvironment {
    @test public async "CreateEnvironment#createEnvironment()"() {
        let pool = getPgPool();
        try {
            await cleanup(pool);
            let res = await setup(pool);
            let result = await createEnvironment(`id=${res.id} token=${res.token}`, "test", "test", "test");
    ...
    @test public async "CreateEnvironment#deprecated()"() {
        let pool = getPgPool();
        try {
            await cleanup(pool);
            let res = await setup(pool);
