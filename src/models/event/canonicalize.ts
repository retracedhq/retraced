...
export default function (event: Event): string {
  // EVENT_ID:ACTION:TARGET_ID:ACTOR_ID:GROUP_ID:SOURCE_IP:IS_FAILURE:IS_ANONYMOUS:FIELDS
  ...
  if (!event.fields) {
    ...
    _.forOwn(event.fields, (value, key) => {
  ...
  console.log(`\nCanonicalization Report\n-----------------------\nevent: ${util.inspect(event, false, 100, true)}\ncanonicalString: '${canonicalString}'\nhash: '${hashResult}'\n`);
