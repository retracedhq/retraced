...
export default async function cleanupPostgres(job: any) {
  ...
  const ingestDelQuery = `DELETE FROM ingest_task WHERE project_id = $1 AND environment_id = $2 AND received < $3`;
  const ingestDelResult = await pgPool.query(ingestDelQuery, [
    projectId,
    environemntId,
    new Date(beforeTimestamp),
  ]);
  logger.info(`Old events deleted from ingest_tasks: ${ingestDelResult.rowCount}`);
  result["ingest_task"] = ingestDelResult.rowCount;
  return result;
}
