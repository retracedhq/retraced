...
export default async function scheduleDailyReportsDue(job) {
  ...
  const offsetHours = offsetsWithLocalTimeDuringUTCHour(
    nextUTCHour,
    reportDueLocalHour
  );
  ...
  for (const r of _.flatten(results)) {
    ...
    logger.info(
      `scheduling environment_day reporting job for environment ${
        r.environment_id
      } at UTC offset ${r.utc_offset} with recipients ${JSON.stringify(
        r.recipients.map(({ email }) => email)
      )}`
    );
