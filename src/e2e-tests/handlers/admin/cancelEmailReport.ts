...
class CancelEmailReport {
  ...
  @test
  public async "CancelEmailReport#cancelEmailReport() throws if env user is not found"() {
    const pool = getPgPool();
    try {
      await cleanup(pool);
      await setup(pool);
  ...
  @test
  public async "CancelEmailReport#cancelEmailReport() throws if token is invalid"() {
    const pool = getPgPool();
    try {
      await cleanup(pool);
      await setup(pool);
