...
async function setup(pool, es: Client) {
  ...
  await protectedRun(
    async () => await pool.query("INSERT INTO retraceduser (id, email) VALUES ($1, $2)", ["tests", "test@test.com"])
  );
  await protectedRun(
    async () => await pool.query("INSERT INTO retraceduser (id, email) VALUES ($1, $2)", ["test1", "test1@test.com"])
  );
  await protectedRun(
    async () =>
