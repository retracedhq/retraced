...
async function cleanup(pool, es) {
  ...
  await protectedRun(
    async () =>
      await pool.query(`DELETE FROM admin_token WHERE user_id=$1`, ["tests"])
  );
  await protectedRun(
    async () =>
      await pool.query(`DELETE FROM projectuser WHERE project_id=$1`, ["tests"])
  );
  await protectedRun(
    async () => await pool.query(`DELETE FROM project WHERE id=$1`, ["tests"])
  );
  await protectedRun(
    async () =>
      await pool.query(`DELETE FROM token WHERE environment_id=$1`, ["tests"])
  );
  await protectedRun(
    async () =>
      await pool.query(`DELETE FROM environment WHERE name=$1`, ["tests"])
  );
  await protectedRun(
    async () =>
  ...
  await protectedRun(
    async () =>
      await pool.query(`DELETE FROM deletion_confirmation WHERE id=$1`, [
        "tests",
      ])
  );
}
