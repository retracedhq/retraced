...
class ApproveDeletionConfirmation {
  @test
  public async "ApproveDeletionConfirmation#approveDeletionConfirmation()"() {
    const pool = getPgPool();
    try {
      await cleanup(pool);
      const res = await setup(pool);
      const result = await approveDeletionConfirmation(
  ...
  @test
  public async "ApproveDeletionConfirmation#approveDeletionConfirmation() throws if visible code is invalid"() {
    const pool = getPgPool();
    try {
      await cleanup(pool);
      const res = await setup(pool);
...
async function setup(pool) {
  ...
  await pool.query(
    "INSERT INTO deletion_confirmation (id, deletion_request_id, retraceduser_id, visible_code) VALUES ($1, $2, $3, $4)",
    ["test", "test", "test", "test"]
  );
  return res;
}
