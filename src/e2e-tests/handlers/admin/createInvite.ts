...
async function cleanup() {
  await safeQuery(`DELETE FROM environmentuser WHERE environment_id=$1`, [
    "test",
  ]);
  ...
  await safeQuery(`DELETE FROM retraceduser WHERE id=$1 OR id=$2`, [
    "test",
    "test1",
  ]);
  await safeQuery(`DELETE FROM deletion_request WHERE resource_id=$1`, [
    "test",
  ]);
