...
class CreateDeletionRequest {
  ...
  public async "CreateDeletionRequest#createDeletionRequest() throws if resource kind is invalid"() {
    ...
    try {
      ...
      throw new Error(
        `Expected error 'Unhandled resource kind: 'test'' to be thrown`
      );
    } catch (ex) {
  ...
  public async "CreateDeletionRequest#createDeletionRequest() throws if resource id is invalid"() {
    ...
    try {
      ...
      throw new Error(
        `Expected error 'Environment not found: id='test1'' to be thrown`
      );
    } catch (ex) {
      expect(ex.status).to.equal(400);
  ...
  public async "CreateDeletionRequest#createDeletionRequest() throws if existing request exists"() {
    ...
    try {
      ...
      throw new Error(
        `Expected error 'A deletion request already exists for that resource.' to be thrown`
      );
    } catch (ex) {
      expect(ex.status).to.equal(409);
      expect(ex.err.message).to.equal(
        "A deletion request already exists for that resource."
      );
    } finally {
      await cleanup(pool);
    }
