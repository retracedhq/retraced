...
class UpdateEnterpriseToken {
  @test public async "UpdateEnterpriseToken#updateEnterpriseToken()"() {
    ...
    try {
      ...
      const res = await updateEnterpriseToken("token=test", "test", "test", result.token, "test1", "");
      assert.strictEqual(res.display_name, "test1");
      return assert(res);
    } catch (ex) {
      console.log(ex);
    } finally {
  ...
  public async "UpdateEnterpriseToken#updateEnterpriseToken() throws if token is wrong"() {
    ...
    try {
      ...
      assert.strictEqual(ex.message, "Expected updated row count of 1, got 0");
    } finally {
      await cleanup(pool);
    }
  }
