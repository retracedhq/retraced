...
class CreateActiveSearch {
  ...
  public async "CreateActiveSearch#createActiveSearch() throws search id is not present"() {
    ...
    try {
      ...
      throw new Error(
        `Expected error "Missing required 'saved_search_id' field" to be thrown`
      );
    } catch (ex) {
      expect(ex.status).to.equal(400);
      expect(ex.err.message).to.equal(
        "Missing required 'saved_search_id' field"
      );
    } finally {
      await cleanup(pool);
    }
  ...
  public async "CreateActiveSearch#createActiveSearch() throws search id is invalid"() {
    ...
    try {
      ...
      throw new Error(
        `Expected error "Saved search not found (id=${savedSearchId})' to be thrown`
      );
    } catch (ex) {
      expect(ex.status).to.equal(404);
      expect(ex.err.message).to.equal(
        `Saved search not found (id=${savedSearchId})`
      );
    } finally {
      await cleanup(pool);
    }
