...
class PumpActiveSearch {
  ...
  @test
  public async "PumpActiveSearch#pumpActiveSearch() throws Missing required 'id' parameter"() {
    try {
      await setup({});
      ...
      throw new Error(
        `Expected error "Missing required 'id' parameter" to be thrown`
      );
    } catch (ex) {
  ...
  @test
  public async "PumpActiveSearch#pumpActiveSearch() throws Active search not found"() {
    ...
    try {
      ...
      console.log(res);
      throw new Error(
        `Expected error "Active search not found (id=${activeSearchId})" to be thrown`
      );
    } catch (ex) {
      expect(ex.status).to.equal(404);
      expect(ex.err.message).to.equal(
        `Active search not found (id=${activeSearchId})`
      );
    }
  }
  @test
  public async "PumpActiveSearch#pumpActiveSearch() throws Unknown query descriptor version"() {
    const version = 2;
    ...
    try {
      ...
      console.log(res);
      throw new Error(
        `Expected error "Unknown query descriptor version: ${version}" to be thrown`
      );
    } catch (ex) {
      console.log(ex);
      expect(ex.message).to.equal(
        `Unknown query descriptor version: ${version}`
      );
    }
  }
  @test
  public async "PumpActiveSearch#pumpActiveSearch() throws if active and saved search not found"() {
    const activeSearchId = "test";
