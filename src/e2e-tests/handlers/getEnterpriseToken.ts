...
class GetEnterpriseToken {
  ...
  @test
  public async "GetEnterpriseToken#getEnterpriseToken() throws if token is wrong"() {
    const pool = getPgPool();
    try {
      await cleanup(pool);
      await setup(pool);
  ...
  @test
  public async "GetEnterpriseToken#getEnterpriseToken() throws if auth token does not belong to project"() {
    const pool = getPgPool();
    try {
      await cleanup(pool);
      await setup(pool);
