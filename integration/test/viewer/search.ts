                ...
                context("And the viewer descriptor is exchanged for a session", function () {
                    ...
                    context("When a call is made to the Viewer API GraphQL endpoint for the event", function () {
                        ...
                        specify("Then the response should contain the correct information about the event", function () {
                            expect(responseBody).to.have.deep.property("data.search.edges[0].node.action", "integration.test.api." + randomNumber.toString());
                        });
                    });
                });
            });
                ...
                context("And the viewer descriptor is exchanged for a session", function () {
                    ...
                    context("When a call is made to the Viewer API GraphQL endpoint for the event", function () {
                        ...
                        specify("Then the response should contain the correct information about the event", function () {
                            ...
                            expect(responseBody).to.have.deep.property("data.search.edges[0].node.group.id", "rtrcdqa" + randomNumber.toString());
                    ...
                    context("When a second call is made to the Viewer API GraphQL endpoint", function () {
                        ...
                        specify("Then the most recent event should be an audit.log.view event with the actor specified", function () {
                            expect(responseBody).to.have.deep.property("data.search.edges[0].node.action", "audit.log.view");
                            expect(responseBody).to.have.deep.property("data.search.edges[0].node.group.id", "rtrcdqa" + randomNumber.toString());
                            expect(responseBody).to.have.deep.property("data.search.edges[0].node.crud", "r");
                            expect(responseBody).to.have.deep.property("data.search.edges[0].node.actor.id", "qa@retraced.io");
                            expect(responseBody).to.have.deep.property("data.search.edges[0].node.actor.name", "RetracedQA Employee");
                            expect(responseBody).to.have.deep.property("data.search.edges[0].node.actor.fields[0].key", "department");
                            expect(responseBody).to.have.deep.property("data.search.edges[0].node.actor.fields[0].value", "QA");
                        });
                    });
