...
describe("Viewer API", function () {
  describe("Given the Retraced API is up and running", function () {
    ...
    context(
      "And a call is made to create a viewer description scoped to a group",
      function () {
        let token;
        ...
        context(
          "And the viewer descriptor is exchanged for a session",
          function () {
            let viewerSession;
            beforeEach((done) => {
              chai
            ...
            context(
              "When a call is made to the Viewer API create saved export endpoint",
              function () {
                let responseBody;
                ...
                specify(
                  "Then the response should contain the new saved export.",
                  function () {
                ...
                if (process.env.HMAC_SECRET_VIEWER) {
                  ...
                  context("When the export is rendered", function () {
                    ...
                    specify(
                      "Then the response should contain matching events.",
                      function (done) {
                        this.timeout(Env.EsIndexWaitMs * 3);
                        const desc = {
                        ...
                        const tkn = jwt.sign(
                          desc,
                          process.env.HMAC_SECRET_VIEWER
                        );
                        chai
                          .request(Env.Endpoint)
                          .get(
                            `/viewer/v1/project/${Env.ProjectID}/export/${responseBody.id}/rendered?jwt=${tkn}`
                          )
                          .end((err, res) => {
                            expect(err).to.be.null;
                            expect(res).to.have.property("status", 200);
