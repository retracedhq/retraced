...
describe("Enterprise Search", function () {
    describe("Given the Retraced API is up and running", function () {
        ...
        context("And a call is made into the Retraced API with a standard audit event", function () {
            ...
            context("When a call is made to create an eitapi token using the Publisher API with a custom view_log_action", function () {
                beforeEach((done) => {
                    if (token) {
                        done();
                        return;
                    ...
                    chai.request(Env.Endpoint)
                        ...
                        .send({ display_name: "QA" + randomNumber.toString(), view_log_action: "viewlogs.custom" })
                ...
                context("And the eitapi token is used to call the Enterprise API GraphQL endpoint for the event", function () {
                    let responseBody;
                ...
                context("When a second call is made to the Enterprise API GraphQL endpoint", function () {
                    ...
                    specify("Then the most recent event should be a viewlogs.custom event with the enterprise token specified as the actor", function () {
                        expect(responseBody).to.have.deep.property("data.search.edges[0].node.action", "viewlogs.custom");
                        expect(responseBody).to.have.deep.property("data.search.edges[0].node.group.id", "rtrcdqa1234");
                        expect(responseBody).to.have.deep.property("data.search.edges[0].node.crud", "r");
                        expect(responseBody).to.have.deep.property("data.search.edges[0].node.actor.id", "enterprise:" + token.substring(0, 7));
                        expect(responseBody).to.have.deep.property("data.search.edges[0].node.actor.name", "QA" + randomNumber.toString());
                    });
                });
            });
        });
