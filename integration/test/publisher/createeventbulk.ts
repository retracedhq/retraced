...
describe("Bulk Create Events", function () {
    ...
    describe("Given the Retraced API is up and running", function () {
        ...
        context("And a call is made into the Retraced API with a list of 4 audit events with different actors", () => {
            ...
            context("When a call is made to the GraphQL endpoint for the event", function () {
                ...
                specify("Then the response should contain all four events", function () {
                    ...
                    expect(responseBody).to.have.deep.property("data.search.edges[0].node.actor.id", "LATEST@retraced.io");
                    expect(responseBody).to.have.deep.property("data.search.edges[0].node.created", isoDate(latest));
                    ...
                    expect(responseBody).to.have.deep.property("data.search.edges[1].node.actor.id", "LATER@retraced.io");
                    expect(responseBody).to.have.deep.property("data.search.edges[1].node.created", isoDate(later));
                    ...
                    expect(responseBody).to.have.deep.property("data.search.edges[2].node.actor.id", "NEXT@retraced.io");
                    expect(responseBody).to.have.deep.property("data.search.edges[2].node.created", isoDate(next));
                    ...
                    expect(responseBody).to.have.deep.property("data.search.edges[3].node.action", "integrationbulk" + randomNumber.toString());
                    expect(responseBody).to.have.deep.property("data.search.edges[3].node.actor.id", "NOW@retraced.io");
                    expect(responseBody).to.have.deep.property("data.search.edges[3].node.created", isoDate(now));
                    expect(responseBody).to.have.deep.property("data.search.edges[3].node.group.id", "rtrcdqa1234");
                    expect(responseBody).to.have.deep.property("data.search.edges[3].node.crud", "c");
                    expect(responseBody).to.have.deep.property("data.search.edges[3].node.source_ip", "192.168.0.1");
                });
            });
        });
    });
