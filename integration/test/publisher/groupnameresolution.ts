...
describe("Group Name Resolution", function () {
    ...
    describe("Given the Retraced API is up and running", function () {
        ...
        context("And a call is made into the Retraced API with only a group.id", () => {
            ...
            context("And another call is made into the Retraced API with the same group.id and a group.name", () => {
                ...
                context("When a call is made to the GraphQL endpoint for the first event", function () {
                    ...
                    specify("Then the response should contain a non-null group.id, and the second event's group.name should be populated", function () {
                        expect(responseBody).to.have.deep.property("data.search.edges[0].node.action", "integration" + FIRST_EVENT);
                        expect(responseBody).to.have.deep.property("data.search.edges[0].node.group.id", "rtrcdqa1234" + GROUP_ID);
                        expect(responseBody).to.have.deep.property("data.search.edges[0].node.group.name", "group number " + GROUP_ID);
                    });
                });
                ...
                context("When a call is made to the GraphQL endpoint for the second event", function () {
                    ...
                    specify("Then the response should contain a non-null group.id, and the second event's group.name should be populated", function () {
                        expect(responseBody).to.have.deep.property("data.search.edges[0].node.action", "integration" + SECOND_EVENT);
                        expect(responseBody).to.have.deep.property("data.search.edges[0].node.group.id", "rtrcdqa1234" + GROUP_ID);
                        expect(responseBody).to.have.deep.property("data.search.edges[0].node.group.name", "group number " + GROUP_ID);
                    });
                });
