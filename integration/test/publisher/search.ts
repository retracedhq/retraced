...
describe("Publisher Search", function() {
    describe("Given the Retraced API is up and running", function() {
        ...
        describe(`And 10 events have been reported with actor ${uniqueActorId}`, function() {
            ...
            before(() => Promise.all(events.map((event) => {
            ...
            context(`When searching the publisher GraphQL endpoint for actor_id:${uniqueActorId} with pageSize 3 and fields "action" and "actor.id"`, function() {
              ...
              beforeEach(async function () {
                ...
                specify("Then the connection should contain the first 3 results and metadata about the full results.", function() {
                    expect(connection.currentResults).to.deep.equal([
                      {action: "integration.test.api", actor: {id: uniqueActorId}},
                      {action: "integration.test.api", actor: {id: uniqueActorId}},
                      {action: "integration.test.api", actor: {id: uniqueActorId}},
                    ]);
                ...
                context("When calling connection.nextPage() three times", function() {
                    ...
                    specify("Then the connection should contain the last result and metadata about the full results.", function() {
