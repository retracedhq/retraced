        ...
        context("And at least one eitapi token exists", function () {
          beforeEach((done) => {
            ...
            chai
              ...
              .post(
                `/publisher/v1/project/${Env.ProjectID}/group/rtrcdqa1234/enterprisetoken`
              )
          ...
          context("When one of the tokens is deleted", function () {
            beforeEach(function (done) {
              chai
                .request(Env.Endpoint)
                .delete(
                  `/publisher/v1/project/${Env.ProjectID}/group/rtrcdqa1234/enterprisetoken/${token}`
                )
                .set("Authorization", `token=${Env.ApiKey}`)
                .send({ display_name: "QA" + randomNumber.toString() })
                .end(function (err, res) {
            ...
            context(
              "And that token is used to query the graphql endpoint",
              function () {
                let response;
                beforeEach(function (done) {
                  chai
