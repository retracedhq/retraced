...
pkg:
	...
	sed -i.bak 's/lazyLoad(.\(.\+\).\, opts)/require(".\/api\/\1.js")(opts)/g' node_modules/@elastic/elasticsearch/api/index.js && \
	 `yarn bin`/pkg -t node18-linuxstatic-x64 --options no-deprecation --output api ./build/src/index.js && \
	 `yarn bin`/pkg -t node18-linuxstatic-x64 --options no-deprecation --output retracedctl ./build/src/retracedctl.js && \
	 `yarn bin`/pkg -t node18-linuxstatic-x64 --options no-deprecation --output processor ./build/src/_processor/index.js && \
	 `yarn bin`/pkg -t node18-linuxstatic-x64 --options no-deprecation --output retraceddb ./build/src/_db/runner-lite.js && \
	 `yarn bin`/pkg -t node18-linuxstatic-x64 --options "max_old_space_size=4096,no-deprecation" --output retraceddb4G ./build/src/_db/runner-lite.js && \
	 `yarn bin`/pkg -t node18-linuxstatic-x64 --options "max_old_space_size=8192,no-deprecation" --output retraceddb8G ./build/src/_db/runner-lite.js  &&  \
	 `yarn bin`/pkg -t node18-linuxstatic-x64 --options "max_old_space_size=16384,no-deprecation" --output retraceddb16G ./build/src/_db/runner-lite.js; \
	 fi
