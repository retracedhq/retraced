  ...
  return new Promise((resolve, reject) => {
    ...
    .then((actions) => {
      var updated = _.map(opts.events, (event) => {
        // Title attribute on the object always ~trumps~ <bad word now>, always overrides everything else
        if (event.title) {
        ...
        if (action && action.display_template) {
          ...
          if (event.object) {
            event.display_title = `[**${event.actor.name}**](/project/${opts.project_id}/actor/${opts.actor_id}) performed the action **${event.action}** on [**${event.object.name}**](/project/${opts.project_id}/object/${opts.object_id})`;
          } else {
            event.display_title = `[**${event.actor.name}**](/project/${opts.project_id}/actor/${opts.actor_id}) performed the action **${event.action}**`;
          }
        } else {
